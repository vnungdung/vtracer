...
impl ColorImageConverter {
    ...
    pub fn from_bytes(params: ColorImageConverterParams, image_data: &[u8], width: usize, height: usize) -> Self {
        ...
        let key_color = if Self::should_key_image(&image) {
            Self::find_unused_color_in_image(&image).unwrap_or_default()
        } else {
            Color::default()
        };
        ...
        let svg = Svg::default(); // hoặc từ ID nếu cần
        Self {
